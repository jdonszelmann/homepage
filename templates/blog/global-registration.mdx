...
At [RustNL](https://2024.rustnl.org), I talked to {{ github::user("epage") }} who wants this for the [Testing Devex team](https://github.com/rust-lang/testing-devex-team), 
...
There are currently two libraries that can help you achieve global registration, both of which are maintained by {{ github::user("dtolnay") }}
...
After lots of incredibly helpful discussion with {{ github::user("m-ou-se") }}, 
...
[Later on in this blog post](#externally-implementable-items) I briefly touch on Externally Implementable Items, for which {{ github::user("m-ou-se") }} and I think
...
A fact is that testing devex wants to make custom test frameworks a thing ( {{ github::issue("testing-devex-team#2") }}), 
...
* {{ github::user("epage") }} talked about wanting a dependency to register test fixtures.
...
Many of the things I've written about here come from discussions with {{ github::user("m-ou-se") }}, 
as we thought that a feature she was working on (Externally Implementable Items; EII {{ github::issue("#125418") }}) 
