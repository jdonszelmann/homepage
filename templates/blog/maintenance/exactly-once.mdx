...
That's what I proposed in {{ github::issue("libs-team#676") }}, 
...
The implementation is rather trivial, this is the PR if you're interested anyway: {{ github::pr("#149270") }}.
The problem is that {{ rust::krate("itertools") }} already has a method named `exactly_one`.
...
Though the problem with methods we want on `Iterator`, which already existing in {{ rust::krate("itertools") }} is apparently a common one.
...
We use {{ rust::krate("itertools") }} and its existing `exactly_one` method too, causing the exact same problem users will when we stabilize it.
...
Only 3 weeks ago {{ github::user("amanieu") }} opened the stabilization PR for a feature called `supertrait_item_shadowing` {{ github::pr("rust-lang/rust#148605") }}
...
So, to end off, let me repeat the acknowledgements from under that stabilization PR, by thanking {{ github::user("compiler-errors") }} and {{ github::user("lcnr") }} for making this possible.
